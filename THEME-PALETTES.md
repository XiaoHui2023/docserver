...
- 若要 **像素级** 还原，需 `extra_css` 覆盖 `--md-default-bg-color`、`--md-primary-fg-color` 等（可后续加 `stylesheets/github.css`）。

**docserver 当前默认：J GitHub 灰栏**（`src/mkdocs_config.py` + `theme/github.css`）。
