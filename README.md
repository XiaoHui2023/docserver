...
| 环境 | 脚本 | 作用 |
| --- | --- | --- |
| **在线机**（Ubuntu 等，可访问 PyPI） | `tools/pack.sh` / `tools/pack.bat` | 安装依赖、拉取 theme vendor、PyInstaller 打离线压缩包 |
| **离线机** | `run.sh` / `run.bat` | 解压即用：默认构建包内 `demo/`，产物在 `output/site/` |

在线机用 **`./tools/pack.sh`** 得到 `dist/docserver-offline-*.tar.gz`（含 `run.sh`）；用 **`tools\pack.bat`** 得到 `dist/docserver-offline-win-amd64.zip`（含 `run.bat`）。压缩包内含 `dist/docserver-sync`、`theme/`（含 `mermaid.min.js`）、`demo/` 示例源。解压后运行 `run` 即可（无需先改配置）。

**快速体验**

```bash
tar -xzf dist/docserver-offline-*.tar.gz -C /path/to/docserver && cd /path/to/docserver
bash run.sh
...
`./tools/pack.sh` / `tools\pack.bat` 一步完成 PyInstaller（Linux 可选 staticx）与离线压缩包组装。staticx、spec 与兼容边界见 [PACKAGING.md](PACKAGING.md)。
