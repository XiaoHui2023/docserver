...
| 环境 | 脚本 | 作用 |
| --- | --- | --- |
| **在线机**（Ubuntu 等，可访问 PyPI） | `build.sh` / `build.bat` | 安装依赖、下载 `offline-packages/`、示例构建、生成离线压缩包 |
| **离线机** | `run.sh` / `run.bat` | 解压离线包后一键构建静态站（配置见 `project.yaml`） |

在线机执行 `build.sh` / `build.bat` 后，将 **`release/docserver-offline-*.tar.gz`**（Windows 为 `docserver-offline-win-amd64.zip`）拷到离线机解压。包内含 `release/bin/docserver-sync`、`project.yaml`、`run.sh` / `run.bat`、`theme/`，无需整仓 `src/`。编辑 `project.yaml` 中的 `source`、`out` 后运行 `run` 即可；Web 只需托管 `out` 目录。
