...
- **修复**：`run.sh` 的缓存目录改为实例边界：显式 `CACHE_DIR` 时 pidfile 放在该目录内；未显式设置时按实例使用 `.docserver-cache/<实例ID>/`，实例默认由 `SOURCES/OUT/BASE_URL/SITE_NAME/SITE_URL` 推导，支持 `DOCSERVER_INSTANCE` 显式区分；启动时只停止同一实例，并校验 pidfile 中的 PID 确实是当前包内 `dist/docserver-sync`，避免多实例或 stale PID 误杀其它进程。
- **补充**：`run.sh` 增加 `INTERVAL` 配置项，`WATCH=1` 时传递给 `docserver-sync --interval`，便于离线包用户直接配置长轮询与失败重试等待。
