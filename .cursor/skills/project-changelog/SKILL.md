...
- **修复**：`.pages` 生成忽略源目录 `assets/`（静态资源目录无 `index.md`，否则 awesome-pages 报 `Nav entry "assets" not found`）。`prepare_work` 先 `install_theme_assets` 再写 `.pages`。
...
- **决议**：`run.sh` / `run.bat` 仅调用 `release/bin/docserver-sync`（`--watch`），不回退 `.venv` / `python src`；配置由 `project.yaml` 载入。离线包不含 `pyproject.toml`；frozen 根目录仅认 `theme/` + `project.yaml`。已去掉 `offline-packages` 下载步骤。
