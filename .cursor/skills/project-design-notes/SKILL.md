...
- 示例源：`example/source/`（开发预览）；离线包自带 `demo/`（`run` 默认源，`WATCH=0` 构建一次）。在线打包见根目录 `pack.sh` / `pack.bat`（`docserver-sync` onefile + 离线压缩包）。`pack.sh` / `run.sh` / `example.sh` 已 Git 跟踪可执行位（Unix clone 可 `./`；Windows 仍用 `.bat` 或 `bash`）。
