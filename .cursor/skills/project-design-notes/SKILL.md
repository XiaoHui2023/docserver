...
| `theme/` | 站点 CSS/JS、配色方案 | 否；构建时复制进 `{out}.work/docs/` 再进 `dist/` |
| `src/` | 构建逻辑、`mkdocs.yml` 生成 | 否；由 `python src` 在构建时执行 |
| `dist/` | 静态站点产物 | **http.server 只服务此目录** |
| `dist.work/` | 中间工作区（docs + mkdocs.yml） | 否（构建用） |

### 构建链路（单次）

```text
example/source  ──sync──►  dist.work/docs/
theme/          ──copy──►  dist.work/docs/stylesheets|javascripts/
src/            ──生成──►  dist.work/mkdocs.yml
dist.work/      ──mkdocs build──►  dist/
```
