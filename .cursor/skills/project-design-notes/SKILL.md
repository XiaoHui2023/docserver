...
- **输出与缓存分离**：`-o` 仅为可部署静态站点；直接调用 CLI 时构建缓存默认在当前目录 `.docserver-cache/`（可用 `--cache-dir` 指定）；`run.sh` 默认按实例使用 `.docserver-cache/<实例ID>/`，含 `docs/` 与 `mkdocs.yml`。
...
| `.docserver-cache/` | CLI 默认构建缓存；`run.sh` 默认使用 `.docserver-cache/<实例ID>/` | 否（构建用） |
...
- 运行生命周期：`run.sh` 不得把 `docserver-sync` 后台 detach；应让 `docserver-sync` 接管脚本 PID，避免用户 kill 启动脚本后残留服务进程。`run.sh` 的缓存目录是实例边界：显式 `CACHE_DIR` 时 pidfile 放在该目录内；未显式设置时按实例使用 `.docserver-cache/<实例ID>/`，实例默认由源/输出/URL/站点名推导，可用 `DOCSERVER_INSTANCE` 显式指定。只允许停止同一实例且 PID 必须校验为本实例 `docserver-sync`，避免多个 docserver 或 stale PID 互杀。示例预览若同时启动 watch 与 http.server，必须保留 shell trap 并在退出时清理两棵子进程树，不得用 `exec http.server` 绕过 trap。Python 构建子进程必须用可清理的进程组/Windows 进程树策略，父进程收到终止信号或异常退出时不能留下 `mkdocs` 子进程；Windows 启动器优先用 Job Object `KILL_ON_JOB_CLOSE`，并保留 `taskkill /T /F` 兜底。
